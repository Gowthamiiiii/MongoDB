...
let $res_dept := for $comp in $company.departments[]
  where $comp.dname = "Research"
  return $comp.employees

for $emp in $company.employees[],$res in $res_dept[]
where $emp.ssn = $res
return {"fname": $emp.fname|| " " || $emp.lname, "address": $emp.address}