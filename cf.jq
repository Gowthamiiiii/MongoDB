...
let $hno := for $dept in $company.departments[]
for $loc in $dept.locations[]
where $loc = "Houston"
return $dept.dno

let $houston_proj := for $proj in $company.projects[]
where $proj.plocation = "Houston"
return $proj.pnumber

for $comp in $company.employees[]
let $x := $comp.projects[]
where some $ele in $x.pno satisfies $ele = $houston_proj and not(some $dnum in $hno satisfies $dnum eq $comp.worksFor)
return {name : $comp.fname || " " || $comp.lname, address: $comp.address}
